...
# Update time
apt-get install -y cron ntpdate
ntpdate pool.ntp.org
[ ! -e "/var/spool/cron/crontabs/root" -o -z "$(grep ntpdate /var/spool/cron/crontabs/root 2>/dev/null)" ] && { echo "0 */24 * * * $(which ntpdate) pool.ntp.org > /dev/null 2>&1" >> /var/spool/cron/crontabs/root;chmod 600 /var/spool/cron/crontabs/root; }
#[ ! -e "/var/spool/cron/crontabs/root" -o -z "$(grep supervisorctl /var/spool/cron/crontabs/root 2>/dev/null)" ] && { echo "30 5 */7 * * supervisorctl restart caddy > /dev/null 2>&1" >> /var/spool/cron/crontabs/root;chmod 600 /var/spool/cron/crontabs/root; }
